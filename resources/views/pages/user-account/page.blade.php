    ...
    <body>
        <div class="bg-home d-flex align-items-start justify-content-center">
            <div class="container py-1">
                                ...
                                <tbody>
                                    @foreach ($booking as $item)
                                        <tr class="text-center">
                                            ...
                                            <td>{{ $item->note ?? '-' }}</td>
                                            <td>
                                                @if (is_null($item->scheduled_datetime) && $item->payment_status === '2')
                                            ...
                                            <td>
                                                @if ($item->status === '2')
                                                    <a href="{{ route('UserTeachingSchedule', $item->id) }}"
                ...
                @foreach ($booking as $item)
                    <div class="modal fade" id="bookingModal{{ $item->id }}" tabindex="-1"
                        aria-labelledby="bookingModalLabel{{ $item->id }}" aria-hidden="true">
                                ...
                                <form action="{{ route('booking.schedule', $item->id) }}" method="POST">
                                    ...
                                    <div class="modal-body row">
                                        ...
                                        <div class="form-group mb-3 col-md-12">
                                            ...
                                            <select id="scheduled_datetime" name="scheduled_datetime[]"
                                                ...
                                                @foreach ($item->course->teachings as $teaching)
                                                    @php
                                                        $start = \Carbon\Carbon::parse(
                                                            $teaching->course_starttime,
                                                        )->format('H:i');
                                                        $end = \Carbon\Carbon::parse($teaching->course_endtime)->format(
                                                            'H:i',
                                                        );
