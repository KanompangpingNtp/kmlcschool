        ...
        <div class="bg-home d-flex align-items-start justify-content-center">
            <div class="container py-5">
                ...
                <div class="row g-4 justify-content-center justify-content-xl-start align-items-start">
                    <div
                        class="col-lg-7 d-flex flex-column justify-content-start align-items-start p-4 bg-white shadow rounded-4">
                            ...
                            <div class="carousel-inner">
                                @php
                                    $images = $courses->files->filter(function ($file) {
                                        return in_array(strtolower(pathinfo($file->file_path, PATHINFO_EXTENSION)), [
                                            'jpg',
                                            'jpeg',
                                            'png',
                                        ]);
                                    ...
                                    @foreach ($images as $index => $image)
                                        <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                                            <img src="{{ asset('storage/' . $image->file_path) }}" class="d-block w-100"
                                                alt="..." style="cursor: pointer" data-bs-toggle="modal"
                                                data-bs-target="#imageGalleryModal" data-index="{{ $index }}">
                                        </div>
                                    @endforeach
                                @else
                                ...
                                <video controls autoplay muted loop style="border-radius: 20px;" class="w-100">
                                    @foreach ($videos as $video)
                                        <source src="{{ asset('storage/' . $video->file_path) }}"
                                            type="video/{{ pathinfo($video->file_path, PATHINFO_EXTENSION) }}">
                                    @endforeach
                    ...
                    @endauth --}}
                        <div class="d-flex gap-2 justify-content-center">
                            <a href="{{ route('TeacherHistoryPage', $courses->user_id) }}" class="btn-detail"
                                style="width: 200px;">
                                ประวัติผู้สอน
                            </a>
                            @auth
                        ...
                        จองคอร์ส
                                ...
                                <a href="#" class="btn-detail" style="width: 200px;" data-bs-toggle="modal"
                                    data-bs-target="#bookingModal{{ $courses->id }}">
                                    จองคอร์ส
                                </a>
                            @endauth
                        </div>
                ...
                <!-- Modal -->
                <div class="modal fade" id="bookingModal{{ $courses->id }}" tabindex="-1"
                    aria-labelledby="bookingModalLabel{{ $courses->id }}" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="bookingModalLabel{{ $courses->id }}">จองคอร์ส:
                                    {{ $courses->course_name }}</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                            </div>
                            <form action="{{ route('BookingCreate', $courses->id) }}" method="POST"
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <select id="amount_times" name="amount_times" class="form-control" required>
                                            ...
                                            @foreach ($courses->amountTimes as $amount)
                                                <option value="{{ $amount->amount_time_hour }}">
                                                    {{ $amount->amount_time_hour }} ชั่วโมง {{ $amount->price }} บาท
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                @foreach ($images as $index => $image)
                                    <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                                        <img src="{{ asset('storage/' . $image->file_path) }}"
                                            class="d-block object-fit-cover" style="object-fit: cover;" alt="...">
                                    </div>
                                @endforeach
                            </div>
