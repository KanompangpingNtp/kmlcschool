        ...
        <div class="">
            <div class="card">
                <div class="card-body">
                            ...
                            @foreach ($booking as $item)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        {{ \Carbon\Carbon::parse($item->booking_date)->format('d/m/Y') }}</td>
                                    <td class="text-center">
                                        @if ($item->teachings)
                                    ...
                                    <td class="text-center">
                                        <button type="button" class="btn btn-success btn-sm" data-bs-toggle="modal"
                                            data-bs-target="#paymentStatusModal{{ $item->id }}">
                                            ชำระแล้ว
                                        </button>
                                    </td>
                                    <td class="text-center">
                                    ...
                                    <td class="d-flex justify-content-center">
                                        @if ($item->payment_status === '1')
                                            <form action="{{ route('BookingHistoryUpdatePayment', $item->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit"
                                                    class="btn btn-primary btn-sm me-1">ยืนยันการชำระเงิน</button>
                                            </form>
                                        @endif

                                        @if ($item->status === '1')
                                            <form action="{{ route('BookingHistoryUpdateStatus', $item->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('PUT')
                                        ...
                                        <form action="{{ route('AdminBookingHistoryDelete', $item->id) }}" method="POST"
                    ...
                    @foreach ($booking as $item)
                        <div class="modal fade" id="slipModal{{ $item->id }}" tabindex="-1"
                            aria-labelledby="slipModalLabel{{ $item->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="slipModalLabel{{ $item->id }}">หลักฐานการโอนเงิน
                                        </h5>
                                    ...
                                    <div class="modal-body text-center">
                                        @if ($item->transfer_slip)
                                            <img src="{{ asset('storage/' . $item->transfer_slip) }}"
                                                class="img-fluid rounded" alt="ใบโอนเงิน">
                                        @else
                        ...
                        <div class="modal fade" id="noteModal{{ $item->id }}" tabindex="-1"
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="noteModalLabel{{ $item->id }}">หมายเหตุ</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="ปิด"></button>
                                    </div>
                    ...
                    @foreach ($booking as $item)
                        <div class="modal fade" id="paymentStatusModal{{ $item->id }}" tabindex="-1"
                            aria-labelledby="paymentStatusModalLabel{{ $item->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="paymentStatusModalLabel{{ $item->id }}">
                                    ...
                                    <div class="modal-body text-center">
                                        @if ($item->transfer_slip)
                                            <img src="{{ asset('storage/' . $item->transfer_slip) }}" alt="หลักฐานการโอน"
