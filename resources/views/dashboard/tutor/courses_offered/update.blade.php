        ...
        <div class="">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('TutorCoursesOfferedUpdate', $course->id) }}" method="POST"
                            ...
                            <select name="subject_id" class="form-select" required>
                                ...
                                @foreach ($subjects as $subject)
                                    <option value="{{ $subject->id }}"
                                        {{ $subject->id == $course->subject_id ? 'selected' : '' }}>
                                        {{ $subject->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="tutor_id" class="form-select" required>
                                ...
                                @foreach ($users as $tutor)
                                    <option value="{{ $tutor->id }}"
                                        {{ $tutor->id == $course->user_id ? 'selected' : '' }}>
                                        {{ $tutor->name }}
                                    </option>
                                @endforeach
                            </select>
                                ...
                                @foreach ($course->files->where('status', 1) as $file)
                                    <div class="form-check mb-2">
                                        <input class="form-check-input" type="checkbox" name="delete_files[]"
                                            value="{{ $file->id }}" id="deleteFile{{ $file->id }}">
                                        <label class="form-check-label" for="deleteFile{{ $file->id }}">
                                            <a href="{{ asset('storage/' . $file->file_path) }}" target="_blank">ไฟล์
                                                {{ $file->file_path }}</a>
                                        </label>
                                    </div>
                                @endforeach
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="course_duration_hour" class="form-control"
                                value="{{ old('course_duration_hour', $course->course_duration_hour) }}" required>
                        </div>

                        <button type="button" class="btn btn-secondary btn-sm mb-3" id="add-amount-time">+
                            เพิ่มจำนวนชั่วโมง</button>
                        ...
                        <div id="amount-time-container">
                            @foreach ($course->amountTimes as $amount)
                                <div class="amount-time-set border p-3 mb-3 rounded">
                                    ...
                                    <input type="number" name="amount_time_hour[]" class="form-control mb-2"
                                        value="{{ $amount->amount_time_hour }}" required>

                                    <label class="form-label">ราคา (บาท)</label>
                                    <input type="number" name="amount_time_price[]" class="form-control"
                                        value="{{ $amount->price }}" required>

                                    <button type="button"
                                        class="btn btn-danger btn-sm mt-2 remove-amount-time">ลบชุดนี้</button>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <template id="amount-time-template">
                            <div class="amount-time-set border p-3 mb-3 rounded">
                                ...
                                <label class="form-label">จำนวนชั่วโมง</label>
                                <input type="number" name="amount_time_hour[]" class="form-control mb-2" required>

                                <label class="form-label">ราคา (บาท)</label>
                                <input type="number" name="amount_time_price[]" class="form-control" required>
                        ...
                        <!-- ปุ่มเพิ่ม -->
                        <button type="button" class="btn btn-secondary btn-sm mb-3 add-schedule-update">+
                            เพิ่มเวลาเรียน</button>
                                ...
                                <div class="teaching-schedule-set border p-3 mb-3 rounded">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="course_day_id[]" class="form-control" required>
                                            ...
                                            @foreach ($courseDay as $day)
                                                <option value="{{ $day->id }}"
                                                    {{ $teaching->course_day_id == $day->id ? 'selected' : '' }}>
                                                    {{ $day->day_name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="row">
                                        <div class="mb-3 col-md-6">
                                            ...
                                            <input type="time" name="course_starttime[]" class="form-control"
                                                value="{{ $teaching->course_starttime }}" required>
                                        </div>
                                        <div class="mb-3 col-md-6">
                                            <label class="form-label">เวลาสิ้นสุดการสอน</label>
                                            <input type="time" name="course_endtime[]" class="form-control"
                                                value="{{ $teaching->course_endtime }}" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="number" step="0.01" name="hourly_rate[]" class="form-control"
                                            value="{{ $teaching->hourly_rate }}" required>
                                    </div>
